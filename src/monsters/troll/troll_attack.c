...
{
    if (rpg->player->grounded &&
        get_dist(rpg->player->pos, monster->pos) < 400) {
        hit_player(rpg, monster->damage, rpg->player);
        rpg->player->health.cd = 0;
    }
}

static void attack_2_bis(rpg_t *rpg, entity_t *monster)
{
    sfFloatRect intersection = {0};
    sfFloatRect rect = {monster->pos.x - 60,
        monster->pos.y + 50, 120, 75};

    rect.left += 80 * -monster->dir;
...
static void attack_2(rpg_t *rpg, entity_t *monster)
{
    ...
    ms.pos = monster->pos;
    ms.life = 1;
    ms.damage = monster->damage;
    add_monster(rpg, SLASH, ms);
    (*rpg->monsters)->dir = monster->dir;
    attack_2_bis(rpg, monster);
}

static void attack_3(rpg_t *rpg, entity_t *monster)
{
    sfFloatRect intersection = {0};
    ...
    if (sfFloatRect_intersects(&rpg->player->rect,
        &rect, &intersection) == sfTrue && rpg->player->state != ROLL) {
        hit_player(rpg, monster->damage, rpg->player);
        rpg->player->health.cd = 0;
    }
}
...
void troll_attack(rpg_t *rpg, entity_t *monster)
{
    ...
    if (monster->attack.crit == 2)
        attack_3(rpg, monster);
}
