...
void parse_saves(menu_t *menu)
{
    ...
    sfVector2f *size[] = {&(sfVector2f){700, 185}, &(sfVector2f){700, 185},
        &(sfVector2f){700, 185}};
    sfVector2f *pos[] = {&(sfVector2f){1600 / 2 - 350, 200}, &(sfVector2f){1600 / 2 - 350, 425},
        &(sfVector2f){1600 / 2 - 350, 650}};
    ...
    for (; i < 3; i++) {
        ...
        menu->saves[i]->name = get_value_by_tags(menu->saves[i]->parser->root, ((char *[]){"SAVENAME", NULL}));
        menu->saves[i]->is_write = (menu->saves[i]->name != NULL);
        ...
        menu->saves[i]->button->name = menu->saves[i]->name;
    }
    menu->saves[i] = NULL;
}