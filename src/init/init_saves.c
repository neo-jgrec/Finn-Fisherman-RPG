...
void parse_saves(menu_t *menu)
{
    ...
    size_t i = 0; menu->saves = malloc(sizeof(save_menu_t *) * 300);
    ...
    for (; i < 3; menu->saves[i] = malloc(sizeof(save_menu_t)), i++) {
        ...
        menu->saves[i]->name = get_value_by_tags(menu->saves[i]->parser->root,
        ((char *[]){"SAVENAME", NULL}));
