...
static void set_sprt_posx(rpg_t *rpg, parallax_t *back)
{
    if ((back->pos_x = malloc(sizeof(double) * NB_FILES)) == NULL)
        return;
    for (size_t i = 0; i < NB_FILES; i++)
        back->pos_x[i] = rpg->puzzle->spawn.x;
    return;
}
